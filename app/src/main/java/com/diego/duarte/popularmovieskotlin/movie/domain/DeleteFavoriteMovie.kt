...
class DeleteFavoriteMovie(val repository: Repository,
                          mThreadExecutor: Scheduler,
                          mPostExecutionThread: Scheduler
):
    UseCase<Movie,
            DeleteFavoriteMovie.Params>(mThreadExecutor, mPostExecutionThread) {

    data class Params constructor(val movie: Movie)

    override fun buildUseCaseObservable(params: Params?): Observable<Movie> {
        return repository.deleteMovieAsFavorite(params!!.movie)
    }
}